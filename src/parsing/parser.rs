...
use swc_ecma_parser::{Syntax, EsSyntax};
use swc_ecma_visit::swc_ecma_ast::{Script, EsVersion};

pub fn parse_js(src: String) -> anyhow::Result<Script> {
    let cm: Lrc<SourceMap> = Default::default();
    let fm = cm.new_source_file(
        FileName::Custom("simz.js".into()).into(),
        src 
    );
    ...
    let aux = parse_file_as_script(
        ...
        &mut errs)
        .map_err(|e| {
            anyhow::anyhow!("error parsing script: {:?}", e)
        });
    aux
